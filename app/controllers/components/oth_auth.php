...
*/
	...
	var $user_passw_var        = 'passw';
	var $user_group_var        = 'group_id';
	
	var $user_table       	   = 'customers';
	...
	{	 
		 ...
		 uses('sanitize');
		 $login = $params[$this->user_login_var];
		...
		else
		{
			...
			{	
				...
				$res = $this->controller->{$this->user_model}->save($row); 
			}
			...
			if(method_exists($this->controller,'_afterLogin')) {
				$this->controller->_afterLogin();
			}		
			return 1;
		}		 
	}
	...
	function _saveCookie(&$row)
	{
		$user_id = $row[$this->user_model]['id'];
		$hk = md5($this->hashkey.$user_id);		
		$row["{$this->user_model}"]["session_uniq_code"] = $hk;
		$res = $this->controller->{$this->user_model}->save($row); 
		
		$expire_date = mktime(0,0,0,1,1,2020);
		setcookie('othAuth_'.$this->hashkey, $hk, $expire_date);
	}
    ...
    function check()
	{
		...
		if($this->_validRestrictions())
		{
			   ...
			   $this->Session->check($us))
			{
				...
				if(mysql_num_rows(mysql_query("select * from customers where email = '".$login."' and session_die = '1'"))) 
				{
					mysql_query("update customers set session_die = '0' where email = '".$login."' ");
					$this->logout();
					return false;
				}
			...
			if(!empty($_COOKIE[$us])) {
				...
				$hk = $_COOKIE[$us];
				$conditions = array(
							"{$this->user_model}.session_uniq_code" => "$hk",
							"{$this->user_model}.".$this->user_table_active => 1); 		
				$this->controller->{$this->user_model}->recursive = 5;			
				$row = $this->controller->{$this->user_model}->find($conditions);
				$num_users = (int) $this->controller->{$this->user_model}->findCount($conditions);
	...
	function getMsg($id) 
	{
		...
		case 0:
			{
				return "Пожалуйста, войдите в свой аккаунт!";
			}break;
		case -1:
			{
				 return "Введите email/пароль!";
			}break;
		case -2:
			{
				 return "Неверные email/пароль!";
			}break;
		default:
			{
